...
const Defaults = {
  ...
  "number": {
    ...
    validate: function(value, { min, max, required }) {
      return checkRequired(value, required) || (
        tbd()? message('NaN') : 
...
class Validator {
  ...
  constructor(props, state = {}) {
    ...
    Object.keys(props).map((key) => { 
      ...
      const { toObject, toString, validate } = config[key];
      if (!toObject || !toString || !validate) {
        throw new Error(`Unrecognized property ${key} of type ${type}. You must define your custom property methods or use a predefined property type.`);
      }
    });
    this.config = config;
  }
  ...
  // error is either a String or an Error instance (use message to get the error message), never undefined or null
  validate() {
    const config = this.config;
    const results = Object.keys(config).map((key) => {
      const propertyConfig = config[key];
      try {
        const value = propertyConfig.validate(this.state[key], propertyConfig.attrs);
        return isPromise(value)
          ? value.then((error) => ({ key, error }), error => ({ key, error }))
          : Promise.resolve({ key, error: value })
      } catch (error) {
    ...
    return Promise.all(results).then(function(results) {
      return results.filter(function(validationResult) {
        return !isNullOrUndefined(validationResult.error);
      })
    });
  }
